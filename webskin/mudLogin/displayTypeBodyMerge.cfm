...
<ft:processform action="Merge">
	<ft:processformObjects typename="farLogin">
		<cfset oUser = application.fapi.getContentType("mudUser") />
		...
		<cfif qUsers.recordcount>
			...
			<skin:bubble message="The '#qUsers.userid#' FarCry account has been merged into this Microsoft profile. You will no longer be able to log in with that username / password." />
		<cfelse>
...
<cfif application.security.isLoggedIn() and refindnocase("_MICROSOFTUD$",session.dmProfile.username)>
	<cfoutput>
...
<cfelse>
	<cfoutput><ul id='errorMsg'><li>You must be logged in via Microsoft to use this functionality</li></ul></cfoutput>
</cfif>
